...
class testRecipeFinder extends PHPUnit_Framework_TestCase
{
  ...
  public function testCSVLoader() {
    ...
    $this->assertEquals(0, count(Fridge::getItems()));

    //Check if the CSV was loaded properly
    DataLoader::loadFridgeFromCSV('csv_test.csv');
  ...
  public function testJSONLoader() {
    ...
    //Check if recipe collection is empty
    $this->assertEquals(0, count(RecipeCollection::getItems()));

    //Check if the JSON was loaded properly
    $items = DataLoader::loadRecipesFromJSON('json_test.json');
    $this->assertEquals(2, count(RecipeCollection::getItems()));
  }

  public function testFinderReturns() {
    //We already have sample files
    $finder = new RecipeFinder('csv_test.csv', 'json_test.json');

    //Check if recipe collection still has two items
    $this->assertEquals(2, count(RecipeCollection::getItems()));

    //Check if fridge has 5 items
    $this->assertEquals(5, count(Fridge::getItems()));

    //Based on the sample files, check if the program prints "grilled cheese on toast"
    $suggestions = RecipeCollection::getSuggestions();
